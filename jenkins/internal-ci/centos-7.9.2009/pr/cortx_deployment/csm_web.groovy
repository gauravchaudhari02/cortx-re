            ...
            steps {
                ...
                dir("csm") {
                    ...
                    sh label: 'Build', returnStatus: true, script: '''
                        ...
                        ./cicd/build.sh $VERSION -b $BUILD_NUMBER -t -i
                    '''
        ...
        stage('Release') {
            steps {
                ...
                sh label: 'Additional Files', script:'''
                    mkdir "${DESTINATION_RELEASE_LOCATION}/iso"
                    cortx_prvsnr_preq=$(ls "${CORTX_ISO_LOCATION}" | grep "python36-cortx-prvsnr" | cut -d- -f5 | cut -d_ -f2 | cut -d. -f1 | sed s/"git"//)
                    
                    wget -O ${DESTINATION_RELEASE_LOCATION}/iso/install-${VERSION}-$BUILD_NUMBER.sh https://raw.githubusercontent.com/Seagate/cortx-prvsnr/$cortx_prvsnr_preq/srv/components/provisioner/scripts/install.sh
                '''	
        ...
        stage('Deploy') {
            steps {
                ...
                script {
                        ...
                        def res = build job: "pr-deployment", propagate: false, wait: true, parameters: [
                                string(name: 'CORTX_BUILD', value: "${CORTX_BUILD}"),
                                booleanParam(name: 'CREATE_JIRA_ISSUE_ON_FAILURE', value: false),
                                booleanParam(name: 'AUTOMATED', value: false)
                        ]
                        if(res.result.equals("FAILURE")){
